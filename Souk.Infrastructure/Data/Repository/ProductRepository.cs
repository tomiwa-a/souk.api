...
public class ProductRepository(SoukDbContext context) : IProductRepository
{
    ...
    public async Task<IEnumerable<Product>> GetProductsByWarehouseAsync(int warehouseId, CancellationToken cancellationToken = default)
    {
        return await _context.Products.Where(p => p.WarehouseId == warehouseId).ToListAsync(cancellationToken);
    }
}